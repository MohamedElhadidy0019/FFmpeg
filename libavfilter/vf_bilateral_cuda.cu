...
extern "C"
{
     ...
     * @param src_tex_U         U channel of source image or UV channels if format is nv12
        ...
        float sigma_space, float sigma_color, int window_size)
    {
        ...
        float4 currrent_pixel=make_float4(tex2D<float>(src_tex_Y, x, y) * 255.f,
                                          tex2D<float>(src_tex_U, x/channel_ratio, y/channel_ratio) * 255.f,
        ...
        for(int i=0;i<window_size;i++)
        {
            for(int j=0;j<window_size;j++)
            {
                ...
                {
                    if(!src_tex_V){
                        float2 temp_uv = tex2D<float2>(src_tex_U, r/channel_ratio, c/channel_ratio);
                        ...
                        neighbor_pixel=make_float4(tex2D<float>(src_tex_Y, r, c) * 255.f,
                                                   tex2D<float>(src_tex_U, r/channel_ratio, c/channel_ratio) * 255.f,
