...
__device__ static inline float euclidean_distance(float first_yuv[3], float second_yuv[3])
{
    float ans=0;
    for(int i=0; i<3; i++)
    {
        ans+=powf(first_yuv[i]-second_yuv[i],2);
    }
    return sqrtf(ans);
}
...
// distance=    sqrtf(powf(x - i, 2) + powf(y - j, 2));
__device__ static inline float apply_bilateral(cudaTextureObject_t src_tex_Y,,int width,int height,int x, int y, float sigma_space, float sigma_color, int window_size)
{
